...
[![Run on Google Cloud](https://storage.googleapis.com/cloudrun/button.svg)](https://console.cloud.google.com/cloudshell/editor?shellonly=true&cloudshell_image=gcr.io/cloudrun/button&cloudshell_git_repo=https://github.com/abdybaevae/url-shortener.git)
...
1. If you forked this repo, or cloned it into a different directory from the github structure,
   ...
   ```bash
   $ find . -path ./vendor -prune -o -type f \( -name '*.go' -o -name '*.proto' \) -exec sed -i -e "s;github.com/abdybaevae/url-shortener;yourscmprovider.com/youruser/yourrepo;g" {} +
   ```
